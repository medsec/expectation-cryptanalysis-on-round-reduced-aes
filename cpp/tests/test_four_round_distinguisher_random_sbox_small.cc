...
static void encrypt(const small_aes_ctx_t *aes_context,
    ...
    small_aes_random_sbox_encrypt_rounds_only_sbox_in_final(
        aes_context,
        plaintext,
        ciphertext.state,
...
static size_t perform_experiment(ExperimentContext *context) {
    ...
    for (size_t i = 0; i < context->num_sets_per_key; ++i) {
        ...
        for (size_t j = 0; j < NUM_TEXTS_IN_DELTA_SET; ++j) {
            ...
            encrypt(&cipher_ctx, plaintext, ciphertext, context->sbox_index);
...
static void parse_args(ExperimentContext *context,
    ...
    parser.appName("Test for the Small-AES four-round distinguisher."
                   ...
                   "not for the PRP."
                   "S-box index must be in [0,19], choosing one of 20"
                   "predefined random S-boxes.");
    ...
    parser.addArgument("-b", "--sbox_index", 1, false);
    ...
    try {
        ...
        context->sbox_index = parser.retrieveAsLong("b");

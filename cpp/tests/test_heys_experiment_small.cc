...
static bool has_zero_first_nibble(const small_aes_state_t state) {
    return ((state[0] & 0xF0) == 0);
}

// ---------------------------------------------------------

static void zeroize(small_aes_state_t state) {
    memset(state, 0x00, SMALL_AES_NUM_STATE_BYTES);
}
...
static size_t find_num_collisions(SmallStatesVector &ciphertexts) {
    ...
    for (size_t i = 0; i != num_texts; ++i) {
        const SmallState state = ciphertexts[i];
        xor_arrays(temp, temp, state.state, SMALL_AES_NUM_STATE_BYTES);
    }

/**
 * Implementation of Small-AES with nibbles and the PRIDE S-box.
...
namespace ciphers {
    ...
    __m128i small_aes_pride_sbox_sub_bytes(__m128i state) {
        return vshuffle(SMALL_AES_PRIDE_SBOX, state);
    }

    // ---------------------------------------------------------------------

    __m128i small_aes_pride_sbox_encrypt_round(__m128i state,
                                                 const __m128i round_key) {
        state = small_aes_pride_sbox_sub_bytes(state);
    ...
    void small_aes_pride_sbox_encrypt_rounds_only_sbox_in_final(
        ...
        for (size_t i = 1; i < num_rounds; ++i) {
            state = small_aes_pride_sbox_encrypt_round(state, ctx->key[i]);
        }
    ...
    __m128i
    small_aes_pride_sbox_encrypt_rounds_only_sbox_in_final_with_aes_ni(
        const small_aes_ctx_t *ctx,
        __m128i plaintext,
        const size_t num_rounds) {
    ...
    __m256i small_aes_pride_sbox_encrypt_round_4(__m256i state,
                                                   const __m256i round_key) {
        state = small_aes_custom_sbox_sub_bytes_4(state, SMALL_AES_PRIDE_SBOX);
    ...
    void
    small_aes_pride_sbox_encrypt_rounds_4_only_sbox_in_final(
        const small_aes_ctx_t *ctx,
        ...
        for (size_t i = 1; i < num_rounds; ++i) {
            state = small_aes_pride_sbox_encrypt_round_4(state, keys[i]);
        }
